...
export default class ProductsPage {
    ...
    async verifySearchedProducts() { await expect(this.page.locator("h2.title.text-center")).toContainText("Searched Products") };
    ...
    async verifySuccessMessage() { await expect(this.page.getByText("Your product has been added to cart.")).toBeVisible(); };
    async verifyProductIsInTheCart(product: string) {
        ...
        for (let i = 0; i < count; i++) {
            ...
            if (name.toLowerCase().includes(product.toLowerCase())) {
                console.log(`Product "${product}" is in the cart.`);
                return;
            }
        }

        throw new Error(`Product "${product}" is not found in the cart.`);
    };
    async verifyQuantityOfProduct(product: string, number: string) {
        ...
        for (let i = 0; i < count; i++) {
            ...
            if (name.toLowerCase().includes(product.toLowerCase())) {
                ...
                if (quantity === number) {
                    console.log(`Product "${product}" has the correct quantity of ${number}.`);
                } else {
                    console.log(`Product "${product}" has an incorrect quantity. Expected: ${number}, Found: ${quantity}.`);
                }
                return;
            }
        }
    ...
    async goToCart() { await this.page.locator("a i.fa.fa-shopping-cart").click() };
    async viewCart() { await this.page.locator("p.text-center a u").click() };
    async verifyCartPage() { await expect(this.page.getByText("Shopping Cart", { exact: true })).toBeVisible(); }
    ...
    async verifyNoAccount(){await expect(this.page.locator("div.login-form")).toContainText("Your email or password is incorrect!")}
